...
class CompetitiveBlock(nn.Module):
    def __init__(self, c2, n_competitor, weight=0.8):
        ...
        self.argmax   = nn.Softmax(dim=1) # Channel-wise
        self.argmax_x = nn.Softmax(dim=2) # X-axis
        self.argmax_y = nn.Softmax(dim=3) # Y-axis
    ...
    def forward(self, x):
        ...
        # 2-nd order (More detailed features)
        x    = self.conv2nd(x)
        x2_1 = self.argmax(x)
...
class CLCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.head(x), x # return logits and features