...
def get_argument():
	...
	parser.add_argument('--FVUSM_annotation_file_compare', type=str, default=r'./datasets/annotations_fvusm_compare.pkl')
...
# def create_FVUSM_annotation_new(args):
# 	def iter(root, samples=[], sub2classes={}):
# 		for sub in tqdm(os.listdir(root)):
# 			if not os.path.isdir(os.path.join(root, sub)):
# 				continue
# 			paths = glob.glob(os.path.join(root, sub, '*.jpg'))
...
# 			for path in paths:
# 				samples.append({'path': path, 'label': sub2classes[sub]})
# 		return samples, sub2classes
    
# 	# First process data from both sessions
# 	all_samples = []
# 	sub2classes = {}

# 	# Process first session
# 	all_samples, sub2classes = iter(os.path.join(args.FVUSM_root, '1st_session', 'extractedvein'), all_samples, sub2classes)

# 	# Process second session
# 	all_samples, sub2classes = iter(os.path.join(args.FVUSM_root, '2nd_session', 'extractedvein'), all_samples, sub2classes)

# 	# Shuffle the data after merging both sessions
...
# 	trainingSamples = all_samples[:train_size]
# 	validatingSamples = all_samples[train_size:train_size + val_size]
# 	testingSamples = all_samples[train_size + val_size:]
# 	print('Training samples:', len(trainingSamples))
# # 	print('Validation samples:', len(validatingSamples))
# # 	print('Testing samples:', len(testingSamples))
    
# 	# Save the final annotation files
# 	pickle.dump({
# 		'Training_Set': trainingSamples,
# 		'Validating_Set': validatingSamples,
# 		'Testing_Set': testingSamples,
# 	}, open(args.FVUSM_annotation_file_compare, 'wb'))
...
if __name__== '__main__':
	...
	try:
		...
		# create_FVUSM_annotation_new(args=args)
		print('Done!\n')
